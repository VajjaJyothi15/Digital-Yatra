...
import { LogIn, Compass, UserCheck, ShieldCheck, Eye, EyeOff, Lock, Mail, CheckCircle, Key, User, ArrowRight, X } from 'lucide-react';
...
export default function Login({ onLoginSuccess }) {
  ...
  // Google Account Select Modal State
  ...
  const handleGoogleAccountSubmit = async (selectedEmail, selectedName) => {
    const finalEmail = (selectedEmail || googleEmail || '').trim();
    const finalName = (selectedName || googleName || '').trim() || (finalEmail ? finalEmail.split('@')[0] : 'Google Traveler');
  ...
  return (
    <div className="min-h-[85vh] flex items-center justify-center py-8 sm:py-12 px-4 sm:px-6 lg:px-8 bg-slate-50 relative">
      
      {/* GOOGLE ACCOUNT SELECT MODAL */}
      {showGoogleModal && (
        <div className="fixed inset-0 z-[2600] bg-slate-950/75 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl border border-slate-200 space-y-5 animate-scaleUp">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-extrabold text-slate-900">Sign in with Google Account</h3>
              </div>
              <button 
            ...
            <p className="text-xs text-slate-600 font-medium">
              Choose an account to continue to <strong className="text-slate-900">Digital Yatra</strong>:
            </p>

            {/* Quick Select Saved Google Accounts */}
            <div className="space-y-2">
              <button
                type="button"
                onClick={() => handleGoogleAccountSubmit('tourist.user@gmail.com', 'Google Tourist')}
                className="w-full text-left p-3 rounded-2xl border border-slate-200 hover:border-blue-500 hover:bg-blue-50/50 transition flex items-center justify-between group"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-xs font-bold text-slate-900">Google Account User</div>
                    <div className="text-[11px] text-slate-500">tourist.user@gmail.com</div>
                  </div>
                </div>
            ...
            <div className="relative flex py-1 items-center">
              ...
              <span className="flex-shrink mx-2 text-slate-400 text-[10px] font-bold uppercase">Or enter your Google Email</span>
              <div className="flex-grow border-t border-slate-200"></div>
            </div>

            <form onSubmit={(e) => { e.preventDefault(); handleGoogleAccountSubmit(); }} className="space-y-3">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Your Google Email</label>
                <div className="relative">
                  <input 
                    type="email" 
                    className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-300 text-xs font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-600"
      ...
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl border border-slate-200 p-6 sm:p-8 space-y-6">
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-3 bg-white hover:bg-slate-50 text-slate-800 border border-slate-300 font-extrabold text-xs rounded-xl shadow-xs transition flex items-center justify-center gap-2 cursor-pointer"
        >
